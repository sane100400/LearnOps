...
5. 각 Phase에 이론 + 실습 합쳐 3~5개 모듈을 포함하되, 이론→실습→이론→실습 순으로 교차 배치하세요.
6. 모듈의 type은 "이론", "실습", "워게임" 중 하나. 각 Phase 마지막에 반드시 "워게임" 모듈 1개를 추가하세요.
7. 4~6개 Phase로 구성하세요. (Phase = 학습 단계, 기간은 학습자가 결정)
8. 반드시 아래 JSON 형식으로만 응답하세요. 다른 텍스트 없이 JSON만 출력하세요.
